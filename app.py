    ...
    for isbn in isbns:
        ...
        <div style="display: flex; align-items: center; margin-bottom: 20px;">
            ...
            <div>
                ...
                Average Rating: {avg_rating:.2f}<br>
                <button class="stButton">More Info</button> <!-- More Info Button -->
            </div>
        </div>
...
option = st.radio("🔍 Recommend based on:", ["User ID", "Book Title"], key="recommend_option")

if option == "User ID":
    user_id = st.number_input("Enter User ID:", min_value=1, step=1, key="user_id_input")
    if st.button("Get Recommendations", key="user_id_button"):
        hybrid_recommend(user_id=int(user_id))
else:
    title = st.text_input("Enter Book Title:", key="book_title_input")
    if st.button("Get Recommendations", key="book_title_button"):
